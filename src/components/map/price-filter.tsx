...
export function PriceFilter({ stations, maxPrice, onMaxPriceChange }: PriceFilterProps) {
  ...
  if (min == null || max == null || count < 2 || max - min < 0.005) return null;

  // Step: 0.001 for fine control
  const step = 0.001;
  const sliderMin = Math.floor(min * 1000) / 1000;
  const sliderMax = Math.ceil(max * 1000) / 1000;
  const currentValue = maxPrice ?? sliderMax;

  const filteredCount = maxPrice != null
    ? stations.features.filter((f) => f.properties.price == null || f.properties.price <= maxPrice).length
    : stations.features.length;

  const isActive = maxPrice != null && maxPrice < sliderMax;

  return (
    <div className="absolute bottom-6 right-3 z-10 w-48 rounded-lg border border-black/10 bg-white/90 px-3 py-2.5 shadow-md backdrop-blur-sm">
      ...
      {isActive && (
        <p className="mt-1 text-center text-[9px] text-gray-400">
          {filteredCount} de {stations.features.length} estaciones
        </p>
      )}
    </div>
  );
